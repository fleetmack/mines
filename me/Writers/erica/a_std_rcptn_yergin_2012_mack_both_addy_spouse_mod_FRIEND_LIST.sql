...
CREATE TABLE a_std_rcptn_3
   ...
   spouse_on_other_list    VARCHAR2(1)   NULL,
...
/*** Add Spouse on Other list flag ***/
UPDATE a_std_rcptn_3  b
   SET b.spouse_on_other_list = '1'
 WHERE b.spouse_id IN
    (select id_number
       from activity
      where activity_code = 'C372');
COMMIT;

UPDATE a_std_rcptn_3
   SET spouse_on_other_list = '0'
 WHERE spouse_on_other_list IS NULL;
 /    
...
UPDATE a_std_rcptn_3  r
   ...
   (SELECT 
    (CASE
     WHEN r.id_number IN (select id_number
                             from a_dw_gift_clubs
                            where record_type_code IN ('FR','PA','PX','ST','GS')
                              and (flag_cs_notif = 1 or flag_cs = 1)
                              and id_number NOT IN
     ...
     WHEN r.id_number IN (SELECT id_number 
                           ...
                           WHERE record_type_code IN ('FR','PA','PX','ST','GS')
                             ...
                             AND id_number NOT IN
                                (select id_number from activity where activity_code = 'C372'))   
     THEN 'PC Investing or Greater Friend'
     END)
     FROM dual);                                
